...
- **增量轮（目标 ID 实时标题）**（commit 见 git log，同 4599 实例刷新新 bundle）：
  - 输入框下方实时显示会话标题：默认预填 id 即显示；键入另一已知会话完整 id 后 150ms 内显示其标题（resume/fork 两模式一致）✓
  - 部分键入已知 id：无标题、无提示（不逐键闪现）；停止输入 500ms 后软提示出现（未知 id 同样 500ms settle）✓
  - 验证中发现并修复 `use-locale.ts` 陈旧缓存缺陷：页面以回退语言（en）启动、持久化偏好（zh）晚到时，bind 时缓存的 active 会把该窗口挂载的面板困在 en（表现为刷新后面板 en、tab 标签 zh）。改为每次 snapshot 读取从 live 服务解析后，刷新即 zh，与框架投影一致 ✓；console 零 error；证据截图 C4
...
| 15 | 目标 ID 实时会话标题 | 新建表单目标输入一个已知会话的完整 id（resume 与 fork 模式各试一次） | 输入框下方即时显示该会话标题；部分 id 不显示；未知 id 停止输入 500ms 后显示软提示（与标题互斥）；刷新页面后面板语言与界面一致 | ✅ 已验证 |
