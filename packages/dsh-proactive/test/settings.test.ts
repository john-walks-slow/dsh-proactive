...
test("validateSettingsPatch accepts silent_wake_compaction and rejects non-booleans", () => {
  const out = validateSettingsPatch({ silent_wake_compaction: false });
  assert.ok(!("code" in out));
  assert.deepEqual(out.patch, { silentWakeCompaction: false });
  assert.equal(code(validateSettingsPatch({ silent_wake_compaction: 1 })), "invalid_trigger");
});
