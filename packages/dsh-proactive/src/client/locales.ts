...
export const zh: ProactivePanelCopy = {
  ...
  presetSourceHint: "唤醒投递到运行该 preset 的最近活跃会话；本插件自建的会话永不入选，无合格会话时跳过本次触发（不新建），分支则失败。",
  targetWorkspaceLabel: "目标工作区",
  workspaceHint: "唤醒会投递到该工作区最近更新的会话；本插件自建的会话永不入选，无合格会话时跳过本次触发（不新建）。",
...
export const en: ProactivePanelCopy = {
  ...
  presetSourceHint: "The wake lands in the most recently active session running this preset; sessions this plugin created never match — with no eligible session the fire is skipped (none is created), and fork fails.",
  targetWorkspaceLabel: "Target workspace",
  workspaceHint: "The wake lands in the workspace's most recently updated session; sessions this plugin created never match — with no eligible session the fire is skipped (none is created).",
